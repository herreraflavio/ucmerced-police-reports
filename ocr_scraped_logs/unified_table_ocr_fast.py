...
"""
Unified Table OCR (Optimized for Speed)

- Core features (Grid detection, PDF handling, OCR) preserved.
- Debug outputs and individual cell image saving have been commented out for performance.
"""
    ...
    if detect_header and looks_like_header(hdr):
        ...
        for special in ["Area", "On Campus?"]:
            if special in need and remaining_idx:
                mapping[special] = remaining_idx.pop()
                need.remove(special)
...
ap = argparse.ArgumentParser(
    description="Unified table OCR (Optimized)")
...
ap.add_argument("--lang", default="eng", help="Language")
ap.add_argument("--ocr", choices=["auto", "tesseract",
                "paddle"], default="auto", help="OCR backend")
ap.add_argument("--cell-scale", type=float, default=1.0, help="Upscale factor")
# Kept arguments for compatibility, but logic is disabled/commented
ap.add_argument("--save-cells", action="store_true",
                help="(Disabled for speed)")
ap.add_argument("--save-json", action="store_true", help="Save per-page JSON")
ap.add_argument("--out-dir", default=os.path.join("output", "ocr_output"),
                help="Root output directory")

ap.add_argument("--debug", action="store_true", help="(Disabled for speed)")
...
def log(*m):
    # Logging overhead removed unless absolutely critical
    if args.debug:
        print("[DEBUG]", *m)
...
def ocr_cell_img(crop_bgr, backend="auto", lang="eng", scale=1.0):
    # Only upscale if scale is explicitly not 1.0 (improves speed to skip)
...
def propose_grid_with_openai(img_bgr, model=OPENAI_MODEL):
    ...
    system = (
        "You are an expert at locating tables.\n"
        "Return STRICT JSON ONLY: {table: {bbox: [x1,y1,x2,y2], rows: int, cols: int, x_lines: [], y_lines: []}}"
    )
    user_parts = [
        {"type": "input_text", "text": "Find the dominant table grid."},
    ...
    try:
        ...
        # Fallback omitted for brevity/speed unless strictly needed
        pass
    return {"table": None}
...
def process_single_image(img, *, page_index: int, stem: str, file_dir: str,
    ...
    if boxes and not always_reason:
        ...
        if boxes2:
            ...
            return {"status": "no-grid", "angle": angle, "rows": [], "grid_source": None,
                    "debug_image": None, "page": page_index}
    ...
    for (r, c, x1, y1, x2, y2) in boxes:
        ...
        cell_rel = None
        # --- COMMENTED OUT FOR SPEED: Save individual cell image ---
        ...
        # Skip full meta if JSON not requested to save memory
        if save_json:
            cells_meta.append({
                "r": r, "c": c,
    ...
    # --- COMMENTED OUT FOR SPEED: Save debug image ---
    # if debug_out_path:
    ...
    return {"status": "ok", "rows": data_rows, "grid_source": used_grid, "angle": angle,
            "debug_image": None, "page": page_index, "page_json": page_json}
...
def process_pdf_file(pdf_path, out_root, lang, dpi, always_reason, no_header_detect, ocr_backend, cell_scale, save_cells, save_json):
    ...
    if FORCE_PDF_PAGE == "ALL":
        ...
        for i, img in enumerate(images, start=1):
            ...
            # debug_images.append(dbg_path) # Disabled
